...
class APFSParser(BaseFilesystemParser):
    FS_NAME = "APFS"
    MAGIC_SIGNATURES = [b"NXSB"]
    ...
    def detect(self) -> bool:
        try:
            ...
            if len(boot_sector) < 4:
                return False
            return boot_sector[0:4] == b"NXSB"
        except Exception:
            return False
    
    def initialize(self) -> bool:
        try:
            self.fs_info = {"type": "APFS", "drive_path": self.sector_reader.drive_path}
            self.initialized = True
            return True
        except Exception as e:
            print(f"APFS init error: {e}")
            return False
