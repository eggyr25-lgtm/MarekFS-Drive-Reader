...
class EXT4Parser(BaseFilesystemParser):
    FS_NAME = "EXT4"
    ...
    def initialize(self) -> bool:
        try:
            self.fs_info = {"type": "EXT4", "drive_path": self.sector_reader.drive_path}
            self.initialized = True
            return True
        except Exception as e:
            print(f"EXT4 init error: {e}")
            return False
    ...
    def read_file(self, path: str, size: Optional[int] = None, offset: int = 0) -> bytes:
        # Fallback: read raw image bytes when filesystem parsing is not implemented
